...
export async function seedDatabase(dataSource: DataSource) {
  ...
  const restaurantCategoryRepository =
    dataSource.getRepository(RestaurantCategory);
  const restaurantCategoryMappingRepository = dataSource.getRepository(
    RestaurantCategoryMapping
  );
  ...
  const categoryData = [
    ...
    },
    {
      name: 'Vietnamese',
      slug: 'vietnamese',
      icon_url: 'https://cdn-icons-png.flaticon.com/512/2515/2515268.png',
    },
    {
      name: 'Japanese',
      slug: 'japanese',
      icon_url: 'https://cdn-icons-png.flaticon.com/512/2252/2252080.png',
    },
    {
      name: 'Korean',
      slug: 'korean',
      icon_url: 'https://cdn-icons-png.flaticon.com/512/3816/3816415.png',
    },
    {
      name: 'Italian',
      slug: 'italian',
      icon_url: 'https://cdn-icons-png.flaticon.com/512/3480/3480618.png',
    },
    {
      name: 'Chinese',
      slug: 'chinese',
    ...
    },
    {
      name: 'Thai',
      slug: 'thai',
      icon_url: 'https://cdn-icons-png.flaticon.com/512/2515/2515183.png',
    },
    {
      name: 'Desserts',
      slug: 'desserts',
  ...
  const addresses = [
    ...
    '456 Le Loi Blvd, District 3',
    '789 Tran Hung Dao St, District 5',
    '321 Vo Thi Sau St, District 3',
    '654 Pasteur St, District 1',
    '987 Nguyen Trai St, District 5',
    '147 Ly Thuong Kiet St, District 10',
    '258 Cach Mang Thang 8 St, District 3',
    '369 Dien Bien Phu St, District 3',
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const restaurant = restaurantRepository.create({
      ...
      estimated_prep_time: [15, 20, 25, 30, 35, 40][
  ...
  for (const restaurant of restaurants) {
    for (let day = 0; day < 7; day++) {
      const hours = operatingHoursRepository.create({
        restaurant_id: restaurant.id,
        day_of_week: day,
        open_time: ['08:00:00', '09:00:00', '10:00:00'][
          Math.floor(Math.random() * 3)
        ],
        close_time: ['20:00:00', '21:00:00', '22:00:00', '23:00:00'][
  ...
  for (const restaurant of restaurants) {
    ...
    for (let i = 0; i < numCategories; i++) {
      const menuCategory = menuCategoryRepository.create({
        ...
        description: `Delicious ${categoryNames[
  ...
  const getMenuImageUrl = (itemName: string): string => {
    ...
    if (
      name.includes('roll') ||
      name.includes('goi cuon') ||
      name.includes('cha gio')
    )
  ...
  for (const menuCategory of menuCategories) {
    ...
    for (let i = 0; i < itemsPerCategory; i++) {
      const currentItemName =
        itemNames[(menuItems.length + i) % itemNames.length];
      ...
      const menuItem = menuItemRepository.create({
        ...
        ...(hasOriginalPrice && {
          original_price: (Math.floor(Math.random() * 200) + 50) * 1000,
        }),
  ...
  for (let i = 0; i < 50; i++) {
    const randomVehicleType =
      Math.random() > 0.8 ? VehicleType.CAR : VehicleType.MOTORCYCLE;
    ...
    const driver = driverRepository.create({
      ...
      status:
        Math.random() > 0.1 ? DriverStatus.APPROVED : DriverStatus.PENDING,
    });
    drivers.push(driver);
  }
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const address = userAddressRepository.create({
      ...
      address_line: `${Math.floor(Math.random() * 500) + 1} ${
        ['Nguyen Hue', 'Le Loi', 'Tran Hung Dao', 'Pasteur', 'Vo Thi Sau'][
      ...
      ...(hasDeliveryNote && {
        delivery_note: `Please call when you arrive. ${
          ['Building A', 'Floor 5', 'Ring doorbell'][
            Math.floor(Math.random() * 3)
          ]
        }`,
      }),
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const voucher = voucherRepository.create({
      ...
      ...(hasMaxDiscount && {
        max_discount: (Math.floor(Math.random() * 50) + 20) * 1000,
      }),
  ...
  for (const cart of carts) {
    const restaurantItems = menuItems.filter(
      (item) => item.restaurant_id === cart.restaurant_id
    );
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const restaurant =
      restaurants[Math.floor(Math.random() * restaurants.length)];
    const userAddress =
      userAddresses.find((addr) => addr.user_id === customer.id) ||
      userAddresses[0];
    const subtotal = (Math.floor(Math.random() * 200) + 50) * 1000;
    const deliveryFee = Number(restaurant.delivery_fee);
    const taxAmount = Number((subtotal * 0.1).toFixed(0)); // 10% tax
    const discountAmount =
      Math.random() > 0.7 ? Math.floor(Math.random() * 30) * 1000 : 0;
    ...
    const order = orderRepository.create({
      ...
      ...(hasVoucher && {
        voucher_id: vouchers[Math.floor(Math.random() * vouchers.length)].id,
      }),
      payment_method:
      ...
      estimated_delivery: new Date(
        Date.now() + (Math.floor(Math.random() * 30) + 20) * 60 * 1000
      ),
      ...(hasActualDelivery && {
        actual_delivery: new Date(
          Date.now() + (Math.floor(Math.random() * 40) + 15) * 60 * 1000
        ),
      }),
    });
    orders.push(order);
  ...
  for (const order of orders) {
    const restaurantItems = menuItems.filter(
      (item) => item.restaurant_id === order.restaurant_id
    );
    const numItems = Math.floor(Math.random() * 4) + 1;
  ...
  const deliverableOrders = orders
    .filter((o) =>
      [
  ...
  for (let i = 0; i < deliverableOrders.length; i++) {
    ...
    const deliveryAddress = userAddresses.find(
      (addr) => addr.id === order.delivery_address_id
    );
    ...
    const delivery = deliveryRepository.create({
      ...
      assigned_at: new Date(
        Date.now() - (Math.floor(Math.random() * 60) + 30) * 60 * 1000
      ),
      ...(hasPickedUpAt && {
        picked_up_at: new Date(
          Date.now() - (Math.floor(Math.random() * 30) + 10) * 60 * 1000
        ),
      }),
      ...(hasDeliveredAt && {
      ...
      ...(hasDeliveryProof && {
        delivery_proof_url: `https://picsum.photos/seed/delivery${i}/800/600`,
      }),
    });
  ...
  for (const order of orders) {
    ...
    const payment = paymentRepository.create({
      ...
      transaction_id: `TXN${Math.floor(Math.random() * 1000000)}-${Math.random()
        .toString(36)
        .substring(7)}`,
  ...
  for (const order of deliveredOrders) {
    ...
    const review = reviewRepository.create({
      ...
      ...(hasRestaurantReply && {
        restaurant_reply: 'Thank you for your review!',
      }),
  ...
  for (const review of reviews) {
    ...
    for (let i = 0; i < numImages; i++) {
      const photoIndex =
        (reviewImages.length + i) % REALISTIC_REVIEW_PHOTOS.length;
